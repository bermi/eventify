  ...
  server = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname.startsWith("/bench/") || pathname.startsWith("/dist/")) {
        ...
        const allowedRoot = pathname.startsWith("/bench/") ? benchRoot : distRoot;
...
test("browser benchmarks run", async ({ page }) => {
  ...
  await page.waitForFunction(() => window.__benchDone === true, null, { timeout: 30_000 });

  const error = await page.evaluate(() => window.__benchError ?? null);
  expect(error).toBeNull();

  const results = await page.evaluate(() => window.__benchResults ?? null);
  expect(results).toBeTruthy();
  expect(results.eventify?.cases?.length ?? 0).toBeGreaterThan(0);
