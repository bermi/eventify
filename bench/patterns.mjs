...
export function runPatternBench(overrides = {}) {
  ...
  const makeSuite = (name, patterns) => {
    ...
    runCase("segments (precomputed)", ops, () =>
      runSegments(segmentPatterns, events),
    );
    ...
    if (prefixPatterns.length) {
      runCase(
        "prefix (trailing *)",
        config.events * prefixPatterns.length,
        () => runPrefix(prefixPatterns, events),
      );
    }
  ...
  return {
    ...
    suites: [
      ...
      makeSuite(
        ...
        buildTrailingPatterns(config.patterns),
      ),
    ],
  };
}
...
export function formatPatternBench(result) {
  ...
  lines.push(
    `samples=${result.config.samples} warmup=${result.config.warmup} patterns=${result.config.patterns} events=${result.config.events}`,
  );
  lines.push("");
