...
describe("Eventify", () => {
  ...
  describe("proto noops", () => {
    ...
    it("removeEventListener is a noop on plain objects", () => {
      const target = {};
      const result = Eventify.proto.removeEventListener.call(
        target,
        "event",
        () => {},
      );
  ...
  it("supports dispatchEvent without detail payloads", () => {
    ...
    Object.defineProperty(eventWithUndefinedDetail, "detail", {
      value: undefined,
    });
  ...
  describe("setDefaultSchemaValidator", () => {
    ...
    it("throws when safeParse fails", () => {
      ...
      expect(() =>
        setDefaultSchemaValidator(schema, "ok", { event: "test" }),
      ).toThrow(error);
    });

    it("throws when schema has no parser", () => {
      expect(() =>
        setDefaultSchemaValidator({}, "ok", { event: "test" }),
      ).toThrow(TypeError);
    });
  });
    ...
    it("validates with zod-style parse", () => {
      const schemas = {
        user: {
          ...
          safeParse: () => ({
            success: false,
            error: new Error("safeParse should not run"),
          }),
        },
        coords: {
          parse: (value) => {
  ...
  describe("Async iterator", () => {
    ...
    it("supports for-await loops with abort", async () => {
      ...
      const loop = (async () => {
        for await (const value of emitter.iterate("tick", {
